...
// ── FETCH: servește din cache când offline ──
self.addEventListener('fetch', e => {
  if (e.request.method !== 'GET') return;
  e.respondWith(
    caches.match(e.request).then(cached => {
      return cached || fetch(e.request).then(resp => {
        if (resp && resp.status === 200 && e.request.url.includes('/ProgramSTB/')) {
...
function showTuraNotification(tura, minBefore) {
  const opts = {
