...
async function backup(request, env) {
  ...
  if (c.actiune === 'citesteColeg') {
    try {
      const r = await fetch(urlBroadcast(env, `backup/${nr}.json`), { cache: 'no-store' });
  ...
  if (c.actiune === 'ts') {
    try {
      const r = await fetch(urlBroadcast(env, `backup/${nr}/ts.json`), { cache: 'no-store' });
      const d = r.ok ? await r.json() : null;
      return { status: 200, corp: { ok: true, ts: d } };
    } catch (e) {
      return { status: 502, corp: { ok: false, eroare: 'Nu am putut citi data salvării' } };
    }
  }
