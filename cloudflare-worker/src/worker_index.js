    ...
    case 'utilizatori': {
      ...
      for (const nr of numere) {
        ...
        if (s && s.ultimaVizita) {
          ...
          if (!isNaN(t)) zile = Math.floor((azi - t) / 86400000);
        }
        lista.push({
          nr,
          ultimaVizita: (s && s.ultimaVizita) || null,
          zile,
          depot:     (s && s.depot)     || null,
          platforma: (s && s.platforma) || null,
          instalat:  s ? s.instalat === true : null,
          versiune:  (s && s.versiune)  || null,
          telefoane: p ? abonamentele(p).length : 0,
          sincronizat: (p && p.updat) || null,
          areBackup: !!(chei && chei[nr]),
          blocat: !!blocatiU[nr]
        });
      }
      // Cei fără nicio vizită cunoscută primii, apoi cei mai vechi
      lista.sort((a, b) => (b.zile === null ? 1e9 : b.zile) - (a.zile === null ? 1e9 : a.zile));
