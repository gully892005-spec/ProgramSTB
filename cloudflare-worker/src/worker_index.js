...
async function jetonValid(env, jeton) {
  ...
  let r;
  try {
    r = await fetch(urlBroadcast(env, `adminJetoane/${idJeton(jeton)}.json`), { cache: 'no-store' });
  } catch (e) {
    return 'necunoscut';           // n-am ajuns la Firebase
  }
  if (r.status >= 500) return 'necunoscut';   // Firebase indisponibil
  if (!r.ok) return 'necunoscut';             // 401/403 de la reguli — tot nu e vina jetonului
  let d;
  try { d = await r.json(); } catch (e) { return 'necunoscut'; }
...
async function admin(request, env) {
  ...
  if (!cuParola && stareJeton === 'necunoscut') {
    return { status: 503, corp: { ok: false, eroare: 'Nu am putut verifica jetonul acum. Încearcă din nou.' } };
  }
