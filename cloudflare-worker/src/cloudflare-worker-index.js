...
async function testPush(request, env) {
  ...
  if (!u.sub) return { status: 404, corp: { ok: false, eroare: 'Nu există abonament de notificări. Oprește și pornește notificările din aplicație.' } };

  const cateTure = Array.isArray(u.ture) ? u.ture.length : 0;
  const payload = JSON.stringify({
    title: '✅ Notificare de probă',
    body: `Merge. În cloud sunt ${cateTure} ture pe următoarele 10 zile.`,
    tag: 'test-' + Date.now(),
    url: '/ProgramSTB/'
  });

  try {
    await trimitePush(u.sub, payload, { TTL: 600, urgency: 'high' }, vapid);
  } catch (e) {
    if (e.statusCode === 404 || e.statusCode === 410) {
      await fetch(urlBroadcast(env, `push/${nr}/sub.json`), { method: 'DELETE' }).catch(() => {});
      return { status: 410, corp: { ok: false, eroare: 'Abonamentul a expirat — l-am șters. Oprește și pornește notificările din aplicație.' } };
    }
