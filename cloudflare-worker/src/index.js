...
async function backup(request, env) {
  ...
  if (c.actiune === 'ts') {
    try {
      ...
      const d = await r.json();
      return { status: 200, corp: { ok: true, ts: d } };
    } catch (e) {
      return { status: 502, corp: { ok: false,
...
const ACCES_PAUZA_DUPA_NU = 24 * 3600 * 1000;   // după un refuz, atâta timp nu mai poate cere
...
async function _areVoieLaProgram(env, nrTinta, c) {
  ...
  const inreg = await _accesStare(env, nrTinta, s.cheie);
...
async function acces(request, env) {
  ...
  if (c.actiune === 'cere') {
    ...
    if (vechi && vechi.stare === 'nu' && (acum - (vechi.raspuns || 0)) < ACCES_PAUZA_DUPA_NU) {
      // Un „nu" trebuie să însemne nu. Altfel cererea devine o bătaie la ușă.
      return { status: 429, corp: { ok: false, stare: 'nu',
        eroare: 'Ți-a refuzat cererea. Poți cere din nou peste 24 de ore.' } };
    }
  ...
  if (c.actiune === 'raspunde') {
    ...
    const w = await fetch(urlBroadcast(env, `acces/${nr}/${cheie}.json`), {
      ...
      body: JSON.stringify({ stare: da ? 'da' : 'nu', raspuns: Date.now() })
    });
    if (!w.ok) return { status: 502, corp: { ok: false, eroare: 'Nu am putut salva răspunsul' } };
    return { status: 200, corp: { ok: true, stare: da ? 'da' : 'nu' } };
  }
...
async function _accesAnunta(env, nrTinta, inreg) {
  ...
  const vapid = { subject: 'mailto:admin@programstb', publicKey: env.VAPID_PUBLIC, privateKey: env.VAPID_PRIVATE };
