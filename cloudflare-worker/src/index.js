...
async function trimiteTure(env, toti, numere, now, vapid) {
  ...
  const rezultate = await peGrupuri(numere, async (nr) => {
    ...
    for (const t of u.ture) {
      ...
      if (u.trimis === marcaj) return { sarit: 1 };
...
async function revendica(request, env) {
  ...
  let actual = null;
  try {
    actual = await getJSON(urlBroadcast(env, `proprietar/${nr}.json`));
  } catch (e) {
  ...
  if (actual && actual.dev && actual.dev !== dev) {
    let tel = '';
    try { tel = (await getJSON(`${env.FB_URL}/config/contact.json`)) || ''; } catch (e) {}
    return { status: 200, corp: { ok: false, stare: 'ocupat', tel: String(tel || '') } };
  }

  if (actual && actual.dev === dev) return { status: 200, corp: { ok: true, stare: 'al tau' } };

  const r = await fetch(urlBroadcast(env, `proprietar/${nr}.json`), {
    method: 'PUT', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ dev, la: Date.now() })
  });
  if (!r.ok) return { status: 200, corp: { ok: true, stare: 'nesalvat' } };
  return { status: 200, corp: { ok: true, stare: 'revendicat' } };
}
