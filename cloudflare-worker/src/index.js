...
async function admin(request, env) {
  ...
  switch (cerere.actiune) {
    ...
    case 'repartizare': {
      ...
      const r = await fetch(urlBroadcast(env, `repartizare/${depou}/${luna}.json`), {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
      ...
      return {
        status: 200,
        corp: { ok: true, luna, depou, oameni: Object.keys(pachet.lunara).length, zile: Object.keys(pachet.zilnica).length }
      };
    }
